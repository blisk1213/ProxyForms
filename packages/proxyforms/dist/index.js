...
exports.createProxyFormsClient = createProxyFormsClient;
...
function createFetcher(config, log) {
    return async function _fetch(path, opts) {
        try {
            ...
            console.error("[ProxyForms Error] ", error);
            throw error;
        }
    };
}
function createProxyFormsClient({ blogId, _url, _debug, }) {
    if (typeof window !== "undefined") {
        console.warn("Looks like you're trying to use ProxyForms in the browser. This is not advised. We recommend using server-side rendering frameworks to fetch data.");
    }
    const logger = (0, lib_1.createLogger)(_debug || false);
    const fetcher = createFetcher({
        url: _url || "https://proxyforms.com/api/public",
        blogId,
    }, logger);
    return {
        posts: {
